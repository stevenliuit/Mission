        ...
        def _deco2(request, *args, **kwargs):
            ...
            if session_key:
                ...
                if session and datetime.datetime.now() < session.expire_date:
                    ...
                    if user:
                        logger.debug('Websocket: account [ %s ] request websocket' % user.username)
                        request.user = user
                        if role == 'assets_del':
                            if user.role in ['SU', 'GA']:
                                return func(request, *args, **kwargs)
                            logger.debug('Websocket: account [ %s ] is not assets_del.' % user.username)
                        else:
                            return func(request, *args, **kwargs)
                else:
...
class ExecHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self):
        ...
        role_name = self.get_argument('osauth_del', 'sb')
        ...
        roles = self.perm.get('osauth_del').keys()
        if self.role not in roles:
            self.write_message('No perm that osauth_del %s' % role_name)
            self.close()
        self.assets = self.perm.get('osauth_del').get(self.role).get('asset')

        res = gen_resource({'account': self.user, 'asset': self.assets, 'osauth_del': self.role})
