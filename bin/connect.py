...
from apps.models import Admin, AssetGroup, Server, Log, TtyLog
from apps.models import OsAuth, OsUser, OsAuthAssetGroup, OsAuthServer
from devops.settings import BASE_DIR, LOG_DIR
SSH_KEY_PATH = os.path.join(BASE_DIR, 'keys','osuser')
import ConfigParser



from api import get_object, get_osuser, ServerError, get_server_info, get_user_perm, mkdir, logger,TermLogRecorder
...
class Tty(object):
    ...
    """
    def __init__(self, osuser, server, login_type='ssh'):
        self.username = osuser.user_name
        self.asset_name = server.hostname
        self.ip = None
        self.port = 22
        self.ssh = None
        self.channel = None
    ...
    def get_connect_info(self):
        ...
        """
        asset_info = get_server_info(self.asset)
        user_key = os.path.join(SSH_KEY_PATH, self.asset.osuser.username, self.user.name + '_' + self.asset.osuser.username)  # 获取角色的key，因为ansible需要权限是600，所以统一生成用户_角色key
        #role_pass = CRYPTOR.decrypt(self.role.password)
        connect_info = {'user': self.asset.osuser.username, 'asset': self.asset, 'ip': asset_info.get('ip'),
                        'port': int(asset_info.get('port')), 'username': self.asset.osuser.username,
                         'ssh_key': user_key}
            ...
            if ssh_key and os.path.isfile(ssh_key):
                try:
                    ssh.connect(connect_info.get('ip'),
                                ...
                                #password=connect_info.get('user_pass'),
...
class Nav(object):
    ...
    def __init__(self, user):
        self.user = user
        self.os_user = get_osuser(user)
        self.user_perm = get_user_perm(self.user)
        if NAV_SORT_BY == 'ip':
            self.perm_assets = sorted(self.user_perm.get('asset', []).values(),
                                      key=lambda x: [int(num) for num in x.manage_ip.split('.') if num.isdigit()])
        elif NAV_SORT_BY == 'hostname':
            self.perm_assets = self.natural_sort_hostname(self.user_perm.get('asset', []).values())
        else:
    ...
    def print_nav():
        ...
        打印提示导航
        """
        msg = """\n\033[1;32m###    欢迎使用好贷DevOps跳板机系统   ### \033[0m
    ...
    def search(self, str_r=''):
        ...
        if str_r:
            try:
                ...
                # 匹配 ip, hostname, 备注
                str_r = str_r.lower()
                self.search_result = [asset for asset in self.perm_assets if str_r == str(asset.manage_ip).lower()] or \
                                     [asset for asset in self.perm_assets if str_r in str(asset.manage_ip).lower() \
    ...
    def print_search_result(self):
        ...
        line = '[%-3s] %-16s %-5s  %-' + str(hostname_max_length) + 's %s %s'
        ...
        if hasattr(self.search_result, '__iter__'):
            for index, asset in enumerate(self.search_result):
                # 获取该资产信息
                asset_info = get_server_info(asset)
                # 获取该资产包含的角色
                #role = [str(role.name) for role in self.user_perm.get('asset').get(asset).get('role')]
                print line % (index, asset.manage_ip, asset_info.get('port'),
                              self.truncate_str(asset.hostname), asset.osuser.username, asset.comment)
        print
    ...
    def try_connect(self):
        try:
            ...
            ssh_tty = SshTty(login_user, asset)
            ssh_tty.connect()
